export default {
  ...
  async fetch(request, env) {
    ...
    // App redirects here with ?code=... after user approves Drive access
    if (url.pathname === '/auth/google' && request.method === 'GET') {
      ...
      if (!code) {
        return Response.redirect(`${appUrl}?drive_auth=error&reason=no_code`, 302);
      }
      ...
      try {
        ...
        if (!tokenRes.ok || !tokens.access_token) {
          ...
          return Response.redirect(`${appUrl}?drive_auth=error&reason=token_exchange`, 302);
        }

        // Store refresh token in KV for cron use
        if (tokens.refresh_token) {
          await env.STOCKROOM_USERS.put('drive_refresh_token', tokens.refresh_token);
          console.log('Drive refresh token stored');
        }

        // Store short-lived access token temporarily so app can pick it up
        await env.STOCKROOM_USERS.put('drive_access_token_temp', tokens.access_token, {
    ...
    // ── Retrieve temp access token (called by app on return) ─
    ...
    if (url.pathname === '/set-schedule' && request.method === 'POST') {
      try {
        ...
        // Also store item snapshot as fallback if Drive read fails
        const allItems = [...urgent, ...upcoming];
        if (allItems.length) {
...
// ── Cron: check schedule and send with fresh Drive data ──────
async function cronCheck(env) {
  try {
    ...
    if (!email || !scheduleRaw) { console.log('Cron: no schedule, skipping'); return; }

    const { startDate, startTime, intervalDays } = JSON.parse(scheduleRaw);
    const scheduledAt = new Date(`${startDate}T${startTime || '09:00'}:00`);
    const now         = new Date();

    const nextSendAt = !lastSentAt
      ? scheduledAt
      : new Date(new Date(lastSentAt).getTime() + intervalDays * 86400000);

    if (now < nextSendAt) {
      console.log(`Cron: next send in ${Math.round((nextSendAt - now) / 60000)} mins, skipping`);
      return;
    }

    // ── Try to get fresh items from Drive ──
    let urgentItems   = [];
    let upcomingItems = [];
    let usedDrive     = false;

    const refreshToken = await env.STOCKROOM_USERS.get('drive_refresh_token');
    if (refreshToken) {
      try {
        const freshItems = await getItemsFromDrive(refreshToken, env);
        if (freshItems) {
          urgentItems   = freshItems.filter(i => i.daysLeft <= 7);
          upcomingItems = freshItems.filter(i => i.daysLeft > 7 && i.daysLeft <= 30);
          usedDrive     = true;
          console.log(`Cron: got ${freshItems.length} items from Drive`);
        }
      } catch (err) {
        console.warn('Cron: Drive read failed, falling back to KV snapshot:', err.message);
      }
    }
    ...
    if (!usedDrive) {
      ...
      if (!itemsRaw) { console.log('Cron: no items available, skipping'); return; }
    ...
    if (!urgentItems.length && !upcomingItems.length) {
      console.log('Cron: nothing due, recording send time');
    ...
    if (result.ok) {
      await env.STOCKROOM_USERS.put('last_sent', now.toISOString());
      console.log(`Cron: sent to ${email} (${usedDrive ? 'fresh from Drive' : 'from KV snapshot'})`);
    } else {
...
async function getAccessToken(refreshToken, env) {
  ...
  if (!res.ok || !data.access_token) throw new Error(`Token refresh failed: ${data.error}`);
  return data.access_token;
}

// ── Read items from Drive and compute stock levels ────────────
async function getItemsFromDrive(refreshToken, env) {
  const accessToken = await getAccessToken(refreshToken, env);

  // Find the stockroom file
  ...
  if (!fileRes.ok) throw new Error(`Drive file read failed: ${fileRes.status}`);

  const data = await fileRes.json();
  if (!Array.isArray(data.items)) throw new Error('Invalid Drive file format');

  // Compute daysLeft for each item on the server
  const now = Date.now();
  const results = [];

  for (const item of data.items) {
    if (!item.logs?.length) continue;
    ...
    if (daysLeft <= 30) {
      const history  = (item.logs || []).map(l => {
        const v = parseFloat(String(l.price || '').replace(/[^0-9.]/g, ''));
        return isNaN(v) ? null : v;
      }).filter(v => v !== null);
      const lastPrice = history.length ? `£${history[history.length - 1].toFixed(2)}` : null;
...
async function sendEmail(to, urgentItems, upcomingItems, env) {
  ...
  const makeRows = (itemList) => itemList.map(item => {
    const priceCell = item.lastPrice
      ? `<span style="font-family:monospace;font-weight:700;color:#111">${escHtml(item.lastPrice)}</span>`
      : '<span style="color:#999">—</span>';
    const buyCell = item.url
      ? `<a href="${escHtml(item.url)}" style="display:inline-block;background:#5b8dee;color:#fff;padding:4px 12px;border-radius:6px;text-decoration:none;font-size:12px;font-weight:600">Buy ↗</a>`
      : '<span style="color:#999">—</span>';
    const storePrices = (item.storePrices || []).filter(sp => sp.store && sp.price);
    const storePriceHtml = storePrices.length > 1
      ? `<div style="margin-top:4px;display:flex;gap:6px;flex-wrap:wrap">${storePrices.map(sp =>
          `<span style="font-size:11px;font-family:monospace;padding:2px 7px;border-radius:99px;background:#f0f0f0;color:#555">${escHtml(sp.store)}: ${escHtml(sp.price)}</span>`
        ).join('')}</div>` : '';
    ...
    return `<tr>
      <td style="padding:12px 14px;border-bottom:1px solid #eee;vertical-align:top">
        ...
        ${item.store ? `<div style="font-size:12px;color:#666">${escHtml(item.store)}</div>` : ''}
        ${storePriceHtml}
      </td>
      <td style="padding:12px 14px;border-bottom:1px solid #eee;color:${daysColor};font-family:monospace;font-weight:700;white-space:nowrap;vertical-align:top">${item.daysLeft}d left</td>
  ...
  const tableWrap = (rows) => `
    <table style="width:100%;border-collapse:collapse;font-size:14px;margin-bottom:24px">
      <thead><tr style="background:#f9f9f9">
        ...
        <th style="padding:10px 14px;text-align:left;font-size:11px;color:#999;text-transform:uppercase;border-bottom:2px solid #eee">Time Left</th>
        <th style="padding:10px 14px;text-align:left;font-size:11px;color:#999;text-transform:uppercase;border-bottom:2px solid #eee">Last Price</th>
  ...
  const urgentSection = urgentItems.length ? `
    <div style="background:#fff5f5;border-left:4px solid #e85050;border-radius:4px;padding:12px 16px;margin-bottom:12px">
      <div style="font-weight:700;color:#e85050;font-size:14px">🔴 Order immediately — within 7 days</div>
      <div style="font-size:12px;color:#999;margin-top:2px">${urgentItems.length} item${urgentItems.length !== 1 ? 's' : ''}</div>
    </div>
    ${tableWrap(makeRows(urgentItems))}` : '';
  ...
  const upcomingSection = upcomingItems.length ? `
    <div style="background:#fffbf0;border-left:4px solid #e8a838;border-radius:4px;padding:12px 16px;margin-bottom:12px${urgentItems.length ? ';margin-top:8px' : ''}">
      <div style="font-weight:700;color:#c8861a;font-size:14px">🟡 Order soon — within 30 days</div>
      <div style="font-size:12px;color:#999;margin-top:2px">${upcomingItems.length} item${upcomingItems.length !== 1 ? 's' : ''}</div>
    </div>
    ${tableWrap(makeRows(upcomingItems))}` : '';
...
<body style="margin:0;padding:0;background:#f5f5f5;font-family:system-ui,sans-serif">
  <div style="max-width:600px;margin:32px auto;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.08)">
    ...
    <div style="background:#f9f9f9;padding:16px 28px;font-size:12px;color:#999;text-align:center;border-top:1px solid #eee">
      Sent from STOCKROOM · <a href="${appUrl}" style="color:#999">${appUrl}</a>
  ...
  return resendRes.ok ? { ok: true, id: data.id } : { ok: false, error: data.message || JSON.stringify(data) };
}
